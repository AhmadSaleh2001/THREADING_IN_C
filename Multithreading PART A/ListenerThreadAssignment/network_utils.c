...
void recv_buffer_clean_up(void *arg)
{
...
void close_socket_clean_up(void *arg)
{
	int socket = *((int *)arg);
	close(socket);
}
...
_udp_server_create_and_start(void *arg)
{
	...
	pthread_cleanup_push(recv_buffer_clean_up, recv_buffer);
	pthread_cleanup_push(close_socket_clean_up, (void *)(&udp_sock_fd));
...
int send_udp_msg(char *dest_ip_addr,
