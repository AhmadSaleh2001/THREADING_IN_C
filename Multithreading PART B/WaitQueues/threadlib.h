...
typedef bool *(*wait_queue_function)(void *app_arg, pthread_mutex_t **app_mutex);
...
typedef struct wait_queue_
{
  ...
  // for each some part of logic in the app, we need to store their mutex
...
thread_t *
wait_queue_test_and_wait(wait_queue_t *wq, wait_queue_function wait_queue_block_fn_cb, void *arg);
